...
class CodeCompletor():
    ...
    def select(self, choice):
        """
        Selects the given choice in the completion popup and scrolls the popup to ensure
        the selected choice is visible.
...
def complete_python(text, line, ch, cache, results):  # pylint: disable=too-many-statements
    ...
    class CompletionFinder(ast.NodeVisitor):
        ...
        to attempt to parse the text, and then visits the resulting AST to extract relevant
        ...
        def inside(self, node):
            ...
            Returns:
                bool: True if the node is located within the current line and column range,
                False otherwise.
            """
            return hasattr(node, "lineno") and node.lineno == line + 1 and \
                node.col_offset <= ch <= node.end_col_offset

        def matches(self, lower_attr, lower_match):
            """
            Checks if a given attribute string matches a search string using a fuzzy matching
            algorithm.
        ...
        def get_attributes(self, obj):
            """
            Retrieves the attributes of the given object, including both properties and
            callable methods.
            """
            ...
            def get_parameters(name):
                ...
                for param in signature.parameters.values():
                    ...
                    param_type = param.annotation.__name__ if param.annotation != \
        ...
        def add_object(self, obj, match):
            """
            Adds the attributes of the given object to the list of completions if they
        ...
        def add_attributes(self, attributes, match):
            """
            Adds the given attributes to the list of completions if they match the
        ...
        def get_value(self, node):
            """
            Evaluates the given AST node and returns its value, using the context dictionary
    ...
    def sort(completions):
        ...
        prefix = sorted([
            attr
            for attr in public
            if attr.startswith(finder.token)
        ], key=lambda s: s.lower())
        rest = sorted([
            attr
            for attr in public
            if not attr.startswith(finder.token)
        ], key=lambda s: s.lower())
        private = sorted([
            attr
            for attr in completions
